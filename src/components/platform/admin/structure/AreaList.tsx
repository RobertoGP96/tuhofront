import { useAreaMutations, useAreas } from '@/hooks/platform/use-areas';
import type { AreaFilterOptions, AreaListItem } from '@/types/area';
import { AlertCircle, Building2, Filter, Loader2, Search } from 'lucide-react';
...
interface AreaListProps {
  ...
  initialFilters?: AreaFilterOptions;
}
...
export const AreaList: React.FC<AreaListProps> = ({
  ...
  const [filters, setFilters] = useState<AreaFilterOptions>(initialFilters);
  ...
  const { 
    ...
    isError, 
    refetch 
  } = useAreas({ page, page_size: pageSize, ...filters });
  ...
  if (isLoading && areas.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-gray-500 animate-pulse font-medium">Cargando áreas...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-4">
        ...
        <p className="text-gray-500 mb-6 max-w-md">No se pudieron cargar las áreas. Por favor, intente de nuevo.</p>
        <button
          onClick={() => refetch()}
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
        ...
        <div className="flex items-center gap-4">
           <div className="text-xs text-gray-400 font-medium">
             Mostrando <span className="text-gray-900 font-bold">{areas.length}</span> de <span className="text-gray-900 font-bold">{total}</span> áreas
           </div>
           <button 
      ...
      {areas.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
          ...
          <h3 className="text-xl font-bold text-gray-400">No se encontraron áreas</h3>
