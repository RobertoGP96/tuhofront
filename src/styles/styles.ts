...
export const ProcedureItem = styled.div<ProcedureItemProps>`
  padding: 10px;
  cursor: pointer;
...
export const StateButton = styled.button<StateButtonProps>`
  position: relative;
  padding: 10px 20px;
  width: 100%;
  border: none;
  ...
  background-color: ${(props) =>
    props.$active
      ? getStateColor(String(props.children), true)
      : getStateColor(String(props.children), false)};
  clip-path: polygon( 0 0, 10% 50%, 0 100%, 90% 100%, 100% 50%, 90% 0);
  margin-right: -15px; /* Superposición para conectar las flechas */
  z-index: ${(props) => (props.$active ? 1 : 0)};

  &:hover {
    background-color: ${(props) => getStateColor(String(props.children), true)};
    color: white;
  }
...
const getStateColor = (state: string, active: boolean): string => {
  switch (state) {
    case "Pendiente":
